...
class LocationSales extends StatefulWidget {
  const LocationSales({Key key}) : super(key: key);

  @override
  _LocationSalesState createState() => _LocationSalesState();
}

class _LocationSalesState extends State<LocationSales> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        //       onPressed: () => _pickDateRange(context,myist)),
        // ],
      ),
      body: Container(
            ...
            return GestureDetector(
                onTap: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => TotalPerLocation(
                                location: myist[index],
                                start: start,
                                end: end,
                              )));
                },
                child: LocationSalesWidget(
                  location: myist[index],
                  start: start,
                  end: end,
                ));
...
class LocationSalesWidget extends StatefulWidget {
  final String location;
  final start, end;
  const LocationSalesWidget(
      {Key key,
  ...
  @override
  _LocationSalesWidgetState createState() => _LocationSalesWidgetState();
}

class _LocationSalesWidgetState extends State<LocationSalesWidget> {
  var numberOfEmployees = 0;
  var numberOfReports = 0;

  var totalfuel = 0, totalLpg = 0, totalLube = 0;
    ...
    document.get().then((value) {
      if (value.docs.length > 0) {
        for (var i = 0; i < value.docs.length; i++) {
          ...
          setState(() {
            totalfuel += value.docs[i].data()['fuel'];
          });
        }
      }
    });
    ...
    document.get().then((value) {
      if (value.docs.length > 0) {
        for (var i = 0; i < value.docs.length; i++) {
          print(widget.location);
          setState(() {
            totalLube += value.docs[i].data()['lube'];
          });
        }
      }
    });
    ...
    document.get().then((value) {
      if (value.docs.length > 0) {
        for (var i = 0; i < value.docs.length; i++) {
          print(widget.location);
          setState(() {
            totalLpg += value.docs[i].data()['lpg'];
          });
        }
      }
    });
                ...
                child: Center(
                  ...
                  children: [
                    Row(
                      ...
                      children: [Text('Fuel'), Text(totalfuel.toString())],
                    ),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [Text('Lube:'), Text(totalLube.toString())],
                    ),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [Text('LPG:'), Text(totalLube.toString())],
                    ),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
...
class TotalPerLocation extends StatefulWidget {
  final String location;
  final start, end;
  ...
  @override
  _TotalPerLocationState createState() => _TotalPerLocationState();
}

class _TotalPerLocationState extends State<TotalPerLocation> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            ...
            .where("stationId", isEqualTo: widget.location)
            .where("accountType",
                isEqualTo: 'Filling Station Attendant #OEEM02C')
            .snapshots(),
        builder: (context, asyncSnapshot) {
          if (asyncSnapshot.hasError) {
            ...
            return Center(
              child: Text('No Records posted yet'),
            );
          } else if (asyncSnapshot.hasData) {
...
class EmployeeSalesCard extends StatefulWidget {
  ...
  @override
  _EmployeeSalesCardState createState() => _EmployeeSalesCardState();
}

class _EmployeeSalesCardState extends State<EmployeeSalesCard> {
  var numberOfReports = 0;
  var totalLpg = 0, totalFuel = 0, totalLube = 0;
                ...
                child: Center(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        Text('Total Lube'),
                        Text(totalLube.toString())
                      ],
                    ),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [Text('Total LPG'), Text(totalLpg.toString())],
                    ),
                  ],
                )),
              ),
